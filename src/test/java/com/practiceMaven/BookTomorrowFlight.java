...
public class BookTomorrowFlight {
	public static void main(String[] args) {
	   ...
	   int i = 0;
	   String flightPrice[] = new String[1000];
	   for(WebElement priceList:price)
	   {
		   
		   flightPrice[i] = priceList.getText().substring(2).replace(",", "");
	   ...
	   int j = 0;
	   int count=0;
	   int fPrice[]= new int[1000];
	   for(WebElement priceList:price)
	   {
		   
		   fPrice[j]=Integer.parseInt(flightPrice[j]);
		   //System.out.println(fPrice[j]);
		   j++;
		   count++;
	   ...
	   System.out.println("Lowest Price is " + fPrice[count-1]);
	   //driver.quit();
